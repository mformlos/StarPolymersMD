...
for i, line in enumerate(f_input): 
    ...
    average[1] += float(data[6]) 
    average[2] += float(data[4])
    average[3] += float(data[5])
    average[4] += float(data[3])
    N += 1
    if (data[0] >= av_step): 
        f_output.write("%i %f %f %f %f %f \n" %(int(data[0]), average[0]/N, average[1]/N, average[2]/N, average[3]/N, average[4]/N)) 
        av_step += av_step
...
title = ["e_pot", "r_gyr", "n_patch", "s_patch", "temperature"]
